	...
	app.phys_world.on('impact', function(event) {
		...
		if (get_pt) {
			...
			if (completed_track || score_by_speed_sections) {
				const step_diff =
					app.cars[c].score.times[app.cars[c].score.times.length - 1]
					- app.cars[c].score.times[app.cars[c].score.times.length - 2];
			
				app.cars[c].score.score += app.constant.sim.time_limit - (step_diff / app.loopControl.phys.iter_per_sec_normal);
			}
		}
		// wall
		else {
			...
			if (app.cars[c].score.chkpts >= 1)
				app.cars[c].score.score += .9 * Math.sqrt(
					(app.cars[c].body.position[0] - app.track.chkpts[app.cars[c].score.chkpts - 1].position[0]) ** 2
