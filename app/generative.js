...
function generateTrack(track, track_data, phys_world, renderer) {
	...
	for (var i = 0; i < track_data.chkpts.length; ++i) {
		...
		var boxShape = new p2.Box({
			width: 1,
			height: chkpt_length,
			collisionGroup: phys_world.collisionGroups.CHKPT,
			collisionMask: phys_world.collisionGroups.CAR,
		...
		track.chkpts[i] = new p2.Body({
			mass: 0,
			position: [
				track_data.chkpts[i][0][0] + chkpt_dx / 2,
				track_data.chkpts[i][0][1] + chkpt_dy / 2
			],
			angle: chkpt_angle
		});
	...
	var i; // required in this scope to be used as the array offset for second set of walls
	for (i = 0; i < track_data.walls1.length - 1; ++i) {
		...
		var boxShape = new p2.Box({
			width: 1,
			height: wall_length,
			collisionGroup: phys_world.collisionGroups.TRACK,
			collisionMask: phys_world.collisionGroups.CAR
		});
		...
		track.walls[i] = new p2.Body({
			mass: 0,
			position: [
				track_data.walls1[i][0] + (wall_dx / 2),
				track_data.walls1[i][1] + (wall_dy / 2)
			],
			angle: wall_angle
		});
	...
	for (var j = 0; j < track_data.walls2.length - 1; ++j) {
		...
		track.walls[j + i] = new p2.Body({
			mass: 0,
			position: [
				track_data.walls2[j][0] + wall_dx / 2,
				track_data.walls2[j][1] + wall_dy / 2
			],
			angle: wall_angle
		});
	...
	// miscWalls
	for (i = 0; i < track_data.miscWalls.length; ++i) {
		...
		track.walls[i] = new p2.Body({
			mass: 0,
			position: [
				track_data.miscWalls[i][0][0] + wall_dx / 2,
				track_data.miscWalls[i][0][1] + wall_dy / 2
			],
			angle: wall_angle
		});
...
function generateCar(car, track_data, idx, champions, phys_world, renderer, carControl) {
	var carShape = new p2.Box({
		width: 2,
		height: 4,
		collisionGroup: phys_world.collisionGroups.CAR,
		collisionMask: phys_world.collisionGroups.TRACK | phys_world.collisionGroups.CHKPT
	});
	...
	car.body = new p2.Body({
		...
		position: [
			track_data.start[0],
			track_data.start[1] + Math.random() * 10 - 5
		],
	...
	car.frontWheel = car.vehicle.addWheel({ localPosition: [0, 1.5] });
	car.frontWheel.setSideFriction(6);

	car.backWheel = car.vehicle.addWheel({ localPosition: [0, -1.5] });
	...
	car.prevOutputs.push(carControl.max_reverse_accel/(carControl.max_forward_accel + carControl.max_reverse_accel));  // throttle to 0
