...
function generateTrack(track, track_data, phys_world, renderer) {
    ...
    for (i = 0; i < track_data.walls1.length-1; ++i) {
        ...
        track.walls[i] = new p2.Body({ mass: 0,
                                       position: [track_data.walls1[i][0] + wall_dx/2,
    ...
    for (var j = 0; j < track_data.walls2.length-1; ++j) {
        ...
        track.walls[j+i] = new p2.Body({ mass: 0,
...
function generateCar(car, track_data, idx, champions, phys_world, renderer, carControl) {
    var carShape = new p2.Box({ width: 2,
                                height: 4,
