...
import { Task } from "./systems/task_scheduling/task";
import { TaskScheduler } from "./systems/task_scheduling/task_scheduler";
...
export class SmartCarApp {
	...
	async init() {
		...
		this.renderer.stage.scale.x = 5;
		this.renderer.stage.scale.y = -5;

		this.renderer.stage.addChild(this.track.graphics);

		for (let i = 0; i < 4; i++) {
			const car = new Car(this.physicalWorld, this.renderer);
			car.rigidBody.setTranslation({ x: 85 + i * 10, y: 0 }, true);
			car.rigidBody.setAngvel(1, true);
